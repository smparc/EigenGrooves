...
\begin{itemize}
  ...
  \item \textbf{Maximal Marginal Relevance} --- greedily select
        $\arg\max_d\; \lambda \cdot \mathrm{rel}(d) - (1-\lambda)\max_{s \in S}
        \mathrm{sim}(d,s)$, trading relevance against redundancy with an
        explicit parameter \cite{carbonell1998}.
\end{itemize}
